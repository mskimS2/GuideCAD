...
RECORD_FILE = "../dataset/data/guidecad.json"
...
def collect_test_set_pcs(args, i):
    start = time.time()
    ...
    for data_id in all_data:
        ...
        pc_path = os.path.join("evaluate/gt_pc", data_id + ".ply")
        if not os.path.exists(pc_path):
            continue
        pc = utils.read_ply(pc_path)
        if pc.shape[0] > N_POINTS:
            pc = downsample_pc(pc, N_POINTS)

        pc = normalize_pc(pc)
        # pc = normalize_pc(pc) # openecad는 주석처리해야함
        ref_pcs.append(pc)
    ...
    return ref_pcs, all_data_id


def collect_src_pcs(args, i, all_data_id):
    start = time.time()

    all_paths = sorted(glob.glob(os.path.join(args.src + "/h5_pc", "*.ply")))
...
def main():
    ...
    args.output = args.src + f"/{args.n_test}_eval_gen.txt"
    ...
    for i in range(1, args.times + 1):
        ...
        ref_pcs, all_data_id = collect_test_set_pcs(args, i)
        sample_pcs = collect_src_pcs(args, i, all_data_id)
