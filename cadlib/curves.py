...
class Arc(CurveBase):
    def __init__(
        self, start_point, end_point, center, radius, normal=None, start_angle=None, end_angle=None, ref_vec=None
    ):
    ...
    def __str__(self):
        return "Arc: start({}), end({}), mid({})".format(
            self.start_point.round(4), self.end_point.round(4), self.mid_point.round(4)
        )
    ...
    def from_dict(stat):
        ...
        radius = stat["radius"]
        normal = np.array([stat["normal"]["x"], stat["normal"]["y"], stat["normal"]["z"]])
        start_angle = stat["start_angle"]
        end_angle = stat["end_angle"]
        ref_vec = np.array([stat["reference_vector"]["x"], stat["reference_vector"]["y"]])
    ...
    def from_vector(vec, start_point, is_numerical=True):
        ...
        return Arc(
            start_point, end_point, center_point, radius, start_angle=start_angle, end_angle=end_angle, ref_vec=ref_vec
        )
    ...
    def get_mid_point(self):
        ...
        rot_mat = np.array([[np.cos(mid_angle), -np.sin(mid_angle)], [np.sin(mid_angle), np.cos(mid_angle)]])
    ...
    def numericalize(self, n=256):
        ...
        self.start_angle, self.end_angle = (tmp / (2 * np.pi) * n).round().clip(min=0, max=n - 1).astype(np.int)
...
class Circle(CurveBase):
    ...
    def draw(self, ax, color):
        ap = patches.Circle((self.center[0], self.center[1]), self.radius, lw=1, fill=None, color=color)
        ax.add_patch(ap)
