...
class Extrude(object):
    ...
    def from_vector(vec, is_numerical=False, n=256):
        ...
        res = Extrude(
            profile, sket_plane, int(ext_param[2]), int(ext_param[3]), ext_param[0], ext_param[1], sket_pos, sket_size
        )
        if is_numerical:
    ...
    def to_vector(self, max_n_loops=6, max_len_loop=15, pad=True):
        ...
        ext_param = (
            list(sket_plane_orientation)
            + list(self.sketch_pos)
            + [self.sketch_size]
